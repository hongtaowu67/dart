...
void splatSolidMesh(const dart::Mesh & mesh, dart::Grid3D<float> & sdf) {
    ...
    for (int f = 0; f<mesh.nFaces; f++) {
        ...
        for (int z = std::max(0.0,double((min_z-sdf.offset.z)/sdf.resolution)); z < std::min((double)sdf.dim.z,(double)ceil((max_z-sdf.offset.z)/sdf.resolution)); z++) {
            for (int y = std::max(0.0,double((min_y-sdf.offset.y)/sdf.resolution)); y < std::min((double)sdf.dim.y,(double)ceil((max_y-sdf.offset.y)/sdf.resolution)); y++) {
                for (int x = std::max(0.0,double((min_x-sdf.offset.x)/sdf.resolution)); x < std::min((double)sdf.dim.x,(double)ceil((max_x-sdf.offset.x)/sdf.resolution)); x++) {
